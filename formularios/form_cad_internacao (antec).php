    ...
    <form class="visible" action="<?= $BASE_URL ?>process_internacao.php" id="myForm" method="POST"
        ...
        <div class="form-group row">
            ...
            <div class="form-group col-sm-2">
                ...
                <select class="form-control-sm form-control selectpicker show-tick" data-size="5"
                    data-live-search="true" id="fk_patologia2" name="fk_patologia2[]" multiple>
                    <option value="">Selecione</option>
                    <?php
                    // Ordena o array de pacientes em ordem ascendente pelo nome
...
$("#myForm").submit(function(event) {
    ...
    // Gerar o JSON para os antecedentes
    // const jsonAntec = [];
    // const selectedOptions = document.querySelector("#fk_patologia2")
    //     .selectedOptions; // Obter as opções selecionadas

    // Array.from(selectedOptions).forEach((option) => {
    //     const fk_id_paciente = document.getElementById("fk_paciente_int").value; // ID do paciente
    //     const fk_internacao_ant_int = document.getElementById("id_internacao")
    //         .value; // ID da internação
    //     const fk_internacao_vis = document.getElementById("id_internacao").value; // ID da visita
    //     const intern_antec_ant_int = option.value; // Valor selecionado no select
    ...
    // Adicionar o JSON gerado ao FormData
    // form_data.append("json-antec", JSON.stringify(jsonAntec));

    // console.log("JSON gerado: ", JSON.stringify(jsonAntec)); // Log para depuração

    console.log("Form action URL:", post_url);
    console.log("Form method:", request_method);
    console.log("FormData initialized.");

    // Exibir os campos do FormData por etapas
    // console.log("Iterating over FormData entries:");
    // for (let [key, value] of form_data.entries()) {
    //     console.log(`Key: ${key}, Value: ${value}`);
    // }

    // var fk_int_capeante_js = document.querySelector("#fk_int_capeante");
    // console.log("FK Internação Capeante:", fk_int_capeante_js?.value);
    ...
    $.ajax({
        ...
        data: form_data,
        beforeSend: function() {
            console.log("Sending AJAX request to:", post_url);
        },
        success: function(result) {
            ...
            if (result == '1') {
                ...
                console.log("Updating regIntInput value. Current:", currentRegInt, "New:",
                    newRegInt);
                ...
                console.log("Inputs and selects reset.");
                ...
                console.log("Select inputs reset and refreshed.");
                ...
                console.log("Updated form fields with new values.");
                ...
                console.log("All optional containers hidden.");
                ...
                $("#select_negoc").val("").css({
                    "color": "gray",
                    "font-weight": "normal",
                    "border": "1px",
                    "border-color": "gray",
                });
                console.log("All select inputs reset and styled.");

                $('#alert').removeClass("alert-danger").addClass("alert-success").fadeIn().html(
                    "Cadastrado com sucesso");
                setTimeout(function() {
                    $('#alert').fadeOut('Slow');
                }, 2000);
            } else if (result == '0') {
                $('#alert').removeClass("alert-success").addClass("alert-danger").fadeIn().html(
                    "Paciente possui internação ativa");
                setTimeout(function() {
                    $('#alert').fadeOut('Slow');
                }, 2000);
            }
...
document.getElementById('fk_patologia2').addEventListener('change', function() {
    ...
    const fkPaciente = document.getElementById('fk_paciente_int').value;
    const fkInternacao = document.getElementById('id_internacao').value;
    const fkInternacaoVis = document.getElementById('id_internacao').value; // ajuste conforme necessário

    const jsonAntecedentes = selectedOptions.map(idAntecedente => ({
        fk_id_paciente: fkPaciente,
        fk_internacao_ant_int: fkInternacao,
        fk_internacao_vis: fkInternacaoVis,
